...
# would make strict mode fail while trying to resolve ark-mcp.
set -euo pipefail

requirements_file="$(mktemp "${TMPDIR:-/tmp}/ark-mcp-audit.XXXXXX")"
...
sitecustomize_dir="$(mktemp -d "${TMPDIR:-/tmp}/ark-mcp-audit-site.XXXXXX")"
