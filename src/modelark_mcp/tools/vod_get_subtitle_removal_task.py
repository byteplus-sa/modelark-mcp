...
class VodGetSubtitleRemovalTaskInput(BaseModel):
    ...
    task_id: str = Field(
        ...
        description="Task ID returned by vod_remove_subtitles.",
    )
    persist_output: bool = Field(
        default=True,
        description="Whether to copy a completed cleaned video into durable artifact storage on first successful poll.",
    )
